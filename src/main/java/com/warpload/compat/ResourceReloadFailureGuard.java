...
public final class ResourceReloadFailureGuard {
    ...
    public static <T> CompletableFuture<T> guard(PreparableReloadListener listener, Supplier<CompletableFuture<T>> reload) {
    ...
    private static boolean isCoreClass(String className) {
        return className.startsWith("net.minecraft.")
                ...
                || className.startsWith("com.warpload.")
    ...
    private static boolean isWarploadBoundary(StackTraceElement frame) {
        return frame.getClassName().startsWith("com.warpload.")
                || frame.getMethodName().contains("warpload$");
    }
    ...
    private static <T> CompletableFuture<T> handleFailure(PreparableReloadListener listener, Throwable throwable) {
        Throwable cause = unwrap(throwable);
        if (shouldIsolate(listener, cause)) {
            logIsolated(listener, cause);
    ...
    private static void logIsolated(PreparableReloadListener listener, Throwable throwable) {
        LOGGER.error("WarpLoad isolated resource reload failure in {} ({})",
